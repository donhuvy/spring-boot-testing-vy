pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = 'donhuvy/spring-boot-testing'
    }
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/donhuvy/spring-boot-testing-vy'
            }
        }
        ...
        stage('Trivy Image Scan') {
            steps {
                sh 'trivy image donhuvy/spring-boot-testing:${BUILD_VERSION}'
            }
        }
        ...
        stage('Docker Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub', usernameVariable: 'dockerHubUsername', passwordVariable: 'dockerHubPassword')]) {
                    ...
                    sh "docker push donhuvy/spring-boot-testing:${BUILD_VERSION}"
                }
            }
        }
